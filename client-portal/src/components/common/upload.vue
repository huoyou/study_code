...
<template lang="html">
  <div class="upload" >
     <div v-if="fileList1.length > 0" style="margin-bottom: 10px;">
      <p style="text-align:left;font-size: 12px;">已成功上传的文件列表：</p>
          <Table :data="fileList1"
                    :columns="columns1"
                    style="">
          </Table>
    </div>
    ...
    <p style="text-align:center;color:red;margin-top: 5px;" v-if="errorMsg != ''" id="console">{{errorMsg}}</p>
    <div id="container">
      <div id="selectfiles"  class='btn'><Icon style="font-size: 14px;margin-right: 5px;margin-top: -3px;" type="ios-add-circle-outline" />点击选择附件</div>
  ...
  export default {
    ...
    methods: {
      ...
      sendRequest() {
        ...
        const serverUrl = 'http://192.168.4.81:8080/sys/getOSSPolicy'
