  ...
  <div class="page">
    ...
    <div class="content">
      ...
      <div class="card card1"
           v-if="type == '1' || type == '2' || type == '3'|| type =='4' || type == '5' || type == '6' || type == '7' || type == '10'|| type == '11' ">
      ...
      <!--  前五名供应商合作情况统计表 -->
      <div class="card"
           v-if="type == '1' || type == '2' || type =='4' || type == '5' || type == '6' || type == '7' || type == '10'|| type == '11' ">
      ...
      <div class="card">
        ...
        <upload ref="uploadFile"
                ...
                :fileList1="fileList1"
                :saveOrCommit="saveOrCommit"
                @commit='upload'
                :errorMsg="errorMsg"></upload>
        <file-listing :fileType="list"></file-listing>
        <div class="btn_box">
          <Button style="margin-right: 20px;"
                  class="btn"
                  @click="commit('0')">暂存</Button>
          <Button type="primary"
                  class="btn"
                  @click="commit('1')">提交</Button>
        </div>
      </div>
    </div>
  </div>
...
<script>
  ...
  import { getCompanyinfo, replaceRes, isContained } from '@/libs/util'
  ...
  export default {
    ...
    data() {
      return {
        fileList1: [
          { name: '1', size: '100', status: '已上传', type: 'adadada' }
        ],
        page: '',
        saveOrCommit: '',
        errorMsg: '',
        ...
        // 附件列表
        totalList: [
          { name: '营业执照', id: '1', required: false },
          { name: '营业执照（三证合一）/一般纳税人认定通知书/主要股东以及高管简历', id: '2', required: false },
          { name: '法定代表人身份证复印件', id: '3', required: false },
          { name: '实际控制人身份证复印件', id: '4', required: false },
          { name: '上年度及最近一期增值税纳税申报表', id: '5', required: false },
          { name: '上年度及最近三期增值税纳税申报表、财务报表', id: '6', required: false },
          { name: '上年度经审计及最近3个月的财务报表和增值税纳税申报表', id: '7', required: false },
          { name: '前五大下游客户的订单复印件（每个客户至少各1份）', id: '8', required: false },
          { name: '前五大上游供应商的订单复印件（每个供应商至少各1份）', id: '9', required: false },
          { name: '最近六个月境内、境外主要结算账户银行对账单', id: '10', required: false },
          { name: '近3个月进/出口报关单（国内代采及过往没有进/出口业务的不需提供）', id: '11', required: false },
          { name: '拟代采产品过往三个月的采购合同、采购量统计以及采购价格波动统计表', id: '12', required: false },
          { name: '拟代采产品对应的供应商资质文件', id: '13', required: false },
          { name: '最新的厂房租赁合同或最新的房屋产权证及近3个月房租、水电缴费发票、社保记录及工资表、最近一期的固定资产明细账、固定资产清单及对应的发票', id: '14', required: false },
          { name: '最新的房租租赁合同或最新的房屋产权证及近3个月社保记录及工资表', id: '15', required: false },
          { name: '进销存明细账报表（近3个月料件入库记录统计报表及成品销售出库统计报表）', id: '16', required: false },
          { name: '上年度与最近一期财务报表和二级科目及上年度与近3个月的增值税纳税申报表', id: '17', required: false },
          { name: '近6个月境内、境外主要结算账户的银行流水EXCEL版', id: '18', required: false },
          { name: '委外加工合同以及近3个月的加工厂对账单、出入库记录', id: '19', required: false },
          { name: '近3个月的全部的真实境外订单及信用证及物流对账单、函调通过证明（如有）', id: '20', required: false },
          { name: '近6个月金税系统认证和海关税票认证的进项明细及销项发票清单明细', id: '21', required: false },
          { name: '（境内主体）委托客户营业执照（三证合一)/一般纳税人认定通知书/主要股东及高管简历', id: '22', required: false },
          { name: '（境外主体）委托客户境外公司注册证书、商业登记证、公司章程、近一年的周年申报表 ', id: '23', required: false },
          { name: '近6个月进项发票抵扣认证明细表和销项发票开票明细统计表 ', id: '24', required: false },
          { name: '租赁合同/产权证明、近三个月社保记录及工资表  ', id: '25', required: false },
          { name: '近3个月境外订单', id: '26', required: false },
          { name: '上年度及最近一期财务报表及增值纳税申报表', id: '27', required: false },
          { name: '近3个月境内、境外主要结算账户的银行流水', id: '28', required: false },
          { name: '主要的生产BOM表 及供应商全称名录', id: '29', required: false },
          { name: '与供应链公司最近3个月的出货清单及工厂出货对账单', id: '30', required: false },
        ],
      }
    },
    watch: {
    ...
    mounted() {
      ...
      switch (this.type) {
        case '7':  // 高端制造垫货款
          ...
          this.listIsRequired([3, 4]);
          break;
        case '1':  // 供应链金融
          this.getFileList(arr2);
          this.listIsRequired([1]);
          break;
        case '5':  // 进口（不控票或控票但垫付税款>200万）
          this.getFileList(arr3);
          this.listIsRequired([1, 2, 3, 4]);
          break;
        case '6':   // 高端制造只垫税款
    ...
    methods: {
      ...
      listIsRequired(arr) {
        for (var j = 0; j < arr.length; j++) {
          this.list[arr[j] - 1].required = true;
        }
      },
      getStorage(val) {
        let param = {};
        ...
        getStorageById(param).then(res => {
          console.log('getStorageById', res)
      ...
      commit(val) {
        this.saveOrCommit = val;
        var list = this.$refs.uploadFile.fileList;
        var isSelectedType = list.every((item, index) => {
          return item.fileType != undefined;
        })
        if (isSelectedType) {
          this.$refs.uploadFile.update()
        } else {
          this.errorMsg = '请选择文件类型！';
          setTimeout(() => { this.errorMsg = '' }, 3000);
        }
      },
      upload(val) {
        let fileList = this.$refs.uploadFile.fileList;
        let successList = this.$refs.uploadFile.fileList.filter(el => el.status1 == '7');
        let successListId = successList.map(el=>el.fileType);
        let requiredList = this.list.filter(el => el.required == true);
        let requiredListId = this.list.map(el=>el.id);
        console.log('fileList', fileList);
        console.log('successList', successList);
        console.log('val', val)
        console.log('this.list', this.list)
        console.log('this.requiredList', requiredList)
        if (val == '0') {
          if (this.type == "1" || this.type == "4" || this.type == "5") {
            this.commit1(0);
          }
        } else if (val == '1') {
          console.log(successListId,requiredListId)
          console.log(99,isContained(successListId, requiredListId))
          if (isContained(successListId, requiredListId)) {
            if (this.type == "1" || this.type == "4" || this.type == "5") {
              this.commit1(1);
            }
          }else {
            this.errorMsg = '请先上传必传附件！';
            setTimeout(() => { this.errorMsg = '' }, 3000);
          }
        }
      },
