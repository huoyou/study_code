<template>
  <boot-modal
      ...
      :title="$t('investModal.reservationInvest')"
      size="lg"
    ...
    <div class="reservation-content">
      <form @submit.prevent="confirmReservation">
        ...
        <div class="form-group">
          <label for="" class="form-label block">
            <!--投资金额-->{{$t('investModal.investAmount')}}
          </label>
          <input
              type="number"
              v-model="form.volume"
              class="form-control"
              v-validate="{required: true, decimal: 2, max_value: get_accountBalance.auditVolume, min_value: future.minVolume || 0}"
              name="reservationForm_volume">
          <div class="flex flex-wrap mt-5">
            <span class="color-999 mr-15">
              <!--余额-->{{$t('common.balance')}}：{{get_accountBalance.auditVolume}} USDT
            </span>
            <span class="color-999">
              <!--最小投资金额-->{{$t('investModal.minInvestAmount')}}：{{future.minVolume || 0}}
            </span>
          </div>
        ...
        <div class="form-group">
          <el-button
              ...
              :loading="committing"
              :disabled="committing"
              @click="confirmReservation"><!--立即预约-->{{$t('investModal.reservationNow')}}</el-button>
        </div>
      </form>
    </div>
  </boot-modal>
