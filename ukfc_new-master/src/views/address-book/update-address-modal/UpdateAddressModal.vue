<template>
  <boot-modal
      ...
      :title="$t('addressBookPage.updateAddress')"
      size="lg"
    ...
    <div class="update-address-form">
      <form @submit.prevent="confirmUpdate">
        ...
        <div class="form-group">
          <label for="" class="form-label block"><!--名称-->{{$t('addressBookPage.name')}}</label>
        ...
        <div class="form-group">
          <label for="" class="form-label block">
            <!--备注(选填)-->
            <!--备注-->{{$t('common.remark')}}(<!--选填-->{{$t('common.optional')}})
          </label>
...
<script>
  import index from './update-address';
