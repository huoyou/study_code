...
/* ****************************************** */
  // 文档: https://github.com/941477276/vue-country-intl
  // 效果: https://941477276.github.io/vue-country-intl/dist/index.html
/* ****************************************** */
export default {
  name: 'VueCountryIntl',
  props: {
  ...
  computed: {
    countryList () {
      ...
      if (searchText.length === 0 || !this.searchAble) {
        if(this.disabledCountry != ""){
          return countriesData.filter(item => {
            if(item.name.indexOf(this.disabledCountry) >= 0){
              return false;
            }else{
              return item;
            }
